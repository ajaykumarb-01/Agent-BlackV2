...
def summarize_paper(query: str = "", text: str = "", **kwargs) -> dict:
    ...
    if not text:
        ...
        if papers:
            ...
            content = query
    prompt = f"""Summarize the following ML research content. Return ONLY valid JSON with keys:
- summary (2-3 sentence overview)
- key_contributions (list of 3-5 specific contributions)
- limitations (list of 2-4 limitations)
Content: {content[:3000]}"""
    raw = call_llm(system_prompt="You are a machine learning research summarization expert.", user_prompt=prompt)
